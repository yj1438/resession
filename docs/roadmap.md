...
- [ ] 修复 PTY 子进程自然退出后仍残留在运行列表的问题；
- [ ] 为扫描、搜索、恢复、新建、删除失败提供统一可见错误提示；
- [ ] 验证归档、恢复、移入废纸篓的安全边界和失败回滚；
- [ ] 增加坏 JSONL、缺字段、目录失效、二进制不存在等测试；
- [ ] 对 100、500、1000 个会话记录冷启动、重扫和搜索耗时；
- [ ] 使用真实大会话验证转录性能，确认需要后再引入虚拟滚动；
- [ ] 固化 Windows、macOS 发布产物的启动冒烟检查；
- [ ] CI 固定执行 Rust 测试、TypeScript 编译和 Tauri 构建；
- [x] 更新过期的产品定位与路线图。
